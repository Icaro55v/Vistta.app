...
export function FinanceiroScreen() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-5 mt-6"><h3 className="font-bold mb-4">Fluxo de caixa consolidado</h3><div className="grid grid-cols-1 sm:grid-cols-3 gap-4"><div><span className="text-xs text-slate-400 uppercase font-bold">Vendas realizadas</span><p className="text-xl font-black text-emerald-500">{formatMoney(vendas.reduce((total, venda) => total + Number(venda.total || 0), 0))}</p></div><div><span className="text-xs text-slate-400 uppercase font-bold">Saídas e sangrias</span><p className="text-xl font-black text-rose-500">{formatMoney(totalSaidasCaixa)}</p></div><div><span className="text-xs text-slate-400 uppercase font-bold">Saldo operacional</span><p className="text-xl font-black text-[#4A3AFF]">{formatMoney(vendas.reduce((total, venda) => total + Number(venda.total || 0), 0) - totalSaidasCaixa)}</p></div></div></div>
