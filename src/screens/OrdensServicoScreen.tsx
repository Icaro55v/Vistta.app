...
import { formatMoney, useAppContext } from '../context/AppContext';
...
export function OrdensServicoScreen() {
  ...
  const abrir = (os?: OrdemServico) => {
    setEditando(os || null);
    setForm(os ? { ...os } : { clienteId: '', status: 'aguardando_montagem', previsaoEntrega: '', observacoes: '', receitaId: '', itens: [{ produtoId: '', descricao: '', qtd: 1, valor: 0, tratamento: '' }] });
    setAberta(true);
  };
  ...
  return <div className="flex flex-col h-full">
    <div className="mb-8 flex justify-between items-center gap-4">
      ...
      <button onClick={() => abrir()} className="bg-[#4A3AFF] text-white px-6 py-3 rounded-xl font-semibold flex items-center"><Plus size={20} className="mr-2" /> Nova OS</button>
    </div>
    <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm flex-1 overflow-auto p-2">
      <table className="w-full text-left min-w-[800px]"><thead><tr className="border-b border-slate-100 text-[11px] text-slate-400 uppercase font-semibold"><th className="py-4 px-6">Cliente</th><th className="py-4 px-6">Itens</th><th className="py-4 px-6">Previsão</th><th className="py-4 px-6">Status</th><th className="py-4 px-6 text-right">Total</th><th className="py-4 px-6">Ações</th></tr></thead>
        <tbody className="divide-y divide-slate-50">{ordensServico.map(os => <tr key={os.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30"><td className="py-4 px-6 font-bold">{clientes.find(cliente => cliente.id === os.clienteId)?.nome || 'Cliente não encontrado'}</td><td className="py-4 px-6">{os.itens.length} item(ns)</td><td className="py-4 px-6">{os.previsaoEntrega ? new Date(os.previsaoEntrega).toLocaleDateString('pt-BR') : '-'}</td><td className="py-4 px-6"><select value={os.status} onChange={e => salvarOrdemServico({ ...os, status: e.target.value as StatusOs, atualizadoEm: new Date().toISOString() }, os.id)} className="bg-slate-100 rounded-lg px-2 py-1 text-xs font-bold"><option value="aguardando_montagem">Aguardando Montagem</option><option value="em_laboratorio">Em Laboratório</option><option value="pronto_retirada">Pronto para Retirada</option><option value="entregue">Entregue</option><option value="cancelada">Cancelada</option></select></td><td className="py-4 px-6 text-right font-bold">{formatMoney(os.itens.reduce((total, item) => total + Number(item.valor || 0) * Number(item.qtd || 0), 0))}</td><td className="py-4 px-6"><button onClick={() => abrir(os)} className="p-2 text-slate-400 hover:text-[#4A3AFF]"><Edit2 size={16} /></button></td></tr>)}
